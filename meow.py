import numpy as np
#make sure you do --pip install numpy in the command prompt or terminal, 
# then just run the program by right clicking in vscode and selecting run python file in terminal
import csv
...
def read_data(file, has_labels=True):
    ...
    with open(file, 'r') as f:
        ...
        for row in reader:
            if has_labels:
                # Convert the first four columns to float, and keep the label as is
                features = [float(x) for x in row[:-1]]
                label = row[-1].strip().lower()  # Remove any extra spaces and convert to lowercase
                data.append(features + [label])
            else:
                # Convert all columns to float (test data without labels)
                features = [float(x) for x in row]
                data.append(features)
    return data

train_data = read_data('hw-files/train_copy.csv', has_labels=True)
    ...
    for epoch in range(max_epochs):
        ...
        for row in data:
            ...
            if y * (np.dot(w, x) + b) <= 0:
                ...
                b = b + learning_rate * y
                errors += 1
        print(f"epoch {epoch+1}, errors: {errors}")  # Print epoch and errors
        if errors == 0:
